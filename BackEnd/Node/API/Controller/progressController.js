...
exports.create_progress = function(req, res){
  ...
  var new_prog = new WorkPlan(req.body);
  if (!new_prog.height || !new_prog.weight || !new_prog.age || !new_prog.descript) {
    res.status(400).json({
      "code": 400,
...
exports.update_descript = function(req, res){
  ...
  } else if(!req.body.descript){
    res.status(400).json({
      "code": 400,
      "response":"Missing description in API request."
    });
  } else{
    Prog.updateDescriptByID(req.params.prog_id, req.body.descript, function(err,prog){
      if (err){
        res.send(err);
      } else{
        res.json(prog);
